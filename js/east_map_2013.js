$(document).ready(function() {
  ...
  var mapContainer = d3.select('#east_map_2013')
    .append('svg')
    .attr('width', width)
    .attr('height', height)
    .append('g');
  ...
  mapContainer.append('text')
    ...
    .attr('y', height-15  )
    .attr('class', 'credit')
    .text('Credit: YS Fang (Tindak Malaysia, 2018), Malaysian Election Commission (2018), Malaysiakini (2018)');

  var results = {};

  d3.queue()
    .defer(d3.json, 'data/east_msia.json')
    .defer(d3.csv, 'data/ge13.csv', function(d) {
      if (!results[d['KodPAR']]) {
        results[d['KodPAR']] = {
          winner: '',
          winner_vote: '',
  ...
  function ready(error, map, ge14) {
    ...
    setProjection(map, 'east_msia');

    var const_map = topojson.feature(map, map.objects.east_msia).features;
  ...
  function const_color(d) {
    if (wonCoallitionByKodPAR.get(d.properties.KodPAR) === 'PR')
      return 'ph';
  ...
  function showInfo(d) {
    ...
    winner.text(wonCoallitionByKodPAR.get(d.properties.KodPAR) + ': ' + candidates.winner + ' (' + d3.format(',')(candidates.winner_votes) + ' votes)')
      .attr('class', 'winner');

      majority.text('Majority: ' + d3.format(',')(majorityByKodPAR.get(d.properties.KodPAR)) + ' votes')
      .attr('class', 'majority');

    var losers = candidates.losers;
    losers.forEach(function(candidate, index) {
      loser(index + 1).text(candidate.coallition + ': ' + candidate.name + ' (' + d3.format(',')(candidate.loser_votes) + ' votes)')
        .attr('class', 'losers');
    });
  }
